...
function SiteEventsContainer({ state, actions, apiUri }) {
    ...
    async function refreshEvents() {
        if (!refreshInProgress) {
            try {
                ...
                let events = await telemetryClient.getEvents(null, false);
                let eventData = [];
                ...
                actions.setSiteEvents(eventData);
            }
            finally {
    ...
    */
    function setRefresh() {
        console.log(`Refreshing event display: ${new Date(Date.now()).toLocaleString()}`);
    ...
    let currentState = state.siteEvents;
